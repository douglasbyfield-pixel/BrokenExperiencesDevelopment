...
async function testLocationAPI() {
	...
	try {
		...
		if (data.localityInfo?.informative) {
			data.localityInfo.informative.forEach((info) => {
				...
				if (
					info.order >= 10 &&
					info.name &&
					!info.description?.includes("continent") &&
					!info.description?.includes("sea")
				) {
		...
		if (
			...
			!components.includes(data.countrySubdivision)
		) {
			components.push(data.countrySubdivision);
		}
		if (
			data.principalSubdivision &&
			!components.includes(data.principalSubdivision)
		) {
