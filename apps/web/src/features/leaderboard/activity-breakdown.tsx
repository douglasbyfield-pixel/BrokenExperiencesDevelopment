...
import { Target, Star, Heart, DollarSign } from "lucide-react";
import { LeaderboardUser } from "./leaderboard-card";
...
export function ActivityBreakdown({ users }: ActivityBreakdownProps) {
  const totals = users.reduce(
    (acc, user) => ({
      ...
      experiencesVerified: acc.experiencesVerified + (user.experiencesVerified || 0),
      experiencesSponsored: acc.experiencesSponsored + (user.experiencesSponsored || 0),
    }),
    {
      experiencesAdded: 0,
      experiencesFixed: 0,
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {activities.map((activity) => {
            ...
            return (
              <div
                key={activity.id}
                className={`text-center p-4 rounded-lg ${activity.bgColor}`}
              >
                <Icon className={`h-6 w-6 mx-auto mb-2 ${activity.color}`} />
