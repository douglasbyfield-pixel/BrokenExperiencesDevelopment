...
import { DollarSign, Heart, Star, Target, Trophy } from "lucide-react";
...
export function LeaderboardCard({
	...
	const getRankStyle = (rank: number) => {
		if (rank === 1)
			return "bg-gradient-to-br from-yellow-400 to-yellow-600 text-white shadow-lg shadow-yellow-500/30 scale-110";
		if (rank === 2)
			return "bg-gradient-to-br from-gray-300 to-gray-500 text-white shadow-md shadow-gray-400/30";
		if (rank === 3)
			return "bg-gradient-to-br from-orange-400 to-orange-600 text-white shadow-md shadow-orange-400/30";
		return "bg-muted";
	};
		...
		<Card className="relative overflow-hidden shadow-none">
			...
			<CardHeader className="relative z-10">
				<CardTitle className="flex items-center gap-2">
					<Icon
						className={`h-5 w-5 ${currentCategory?.color === "bg-yellow-500" ? "text-yellow-600" : ""}`}
					/>
				...
				<div className="max-h-96 space-y-2 overflow-y-auto p-6">
					{users.map((user, index) => {
						...
						return (
							<div
								key={user.id}
								className={`group flex items-center justify-between rounded-lg border p-4 transition-all duration-300 hover:scale-[1.02] hover:shadow-md ${
							...
							>
								<div className="flex items-center gap-4">
									{/* Rank */}
									<div
										className={`flex h-10 w-10 items-center justify-center rounded-full transition-all duration-300 ${getRankStyle(rank)} ${
											isTopThree ? "animate-bounce-soft" : ""
										}`}
									>
										<span
											className={`font-bold ${isTopThree ? "text-lg" : "text-sm"}`}
										>
											{rank}
										</span>
									</div>
									...
									<div className="relative">
										<Avatar
											className={`h-10 w-10 ${isTopThree ? "ring-2 ring-offset-2" : ""} ${
												rank === 1
													? "ring-yellow-400"
													: rank === 2
														? "ring-gray-400"
										...
										{isTopThree && (
											<div
												className={`-top-1 -right-1 absolute h-4 w-4 animate-pulse-gaming rounded-full ${
													rank === 1
														? "bg-yellow-400"
														: rank === 2
															? "bg-gray-400"
									...
									<div className="animate-slideIn">
										<h3
											className={`font-semibold text-sm ${isTopThree ? "text-base" : ""}`}
										>
											{user.name}
										</h3>
										{category === "overall" && (
										...
										{category !== "overall" && (
											<p className="text-muted-foreground text-xs">
												...
												{category === "experiencesAdded"
													? "reports"
													: category === "experiencesFixed"
														? "fixes"
														: category === "experiencesVerified"
															? "verifications"
															: "sponsorships"}
											</p>
										)}
								...
								<div className="text-right">
									<div
										className={`font-bold text-lg ${getPointsAnimation(rank)} ${
											isTopThree ? "text-xl" : ""
										}`}
									>
										{category === "overall"
