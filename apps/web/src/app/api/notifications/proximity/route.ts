...
export async function POST(req: Request) {
	try {
		...
		console.log(
			"📍 Checking proximity notifications for experience:",
			experienceId,
		);
      ...
      `)
		...
		const experience: Experience = {
			...experienceData,
			category_name: "General", // Simplified for now
		};
		...
		// Get all users with location data (simplified query)
		...
		if (usersError) {
			...
			return new Response(
				JSON.stringify({ error: "Failed to fetch user locations" }),
				{
					status: 500,
					headers: { "Content-Type": "application/json" },
				},
		...
		console.log(
			"📍 Found users with location data:",
		...
		for (const userLoc of usersWithLocation || []) {
			...
			if (distance <= proximityRadius) {
				...
				console.log(
					`👤 Nearby user found: ${userLoc.user_id} (${distance.toFixed(2)}km away)`,
				);
			}
		}
		...
		const cleanTitle = experience.title.length > 50 
			? experience.title.substring(0, 47) + "..." 
			: experience.title;
		
		const locationParts = experience.address.split(",");
		const shortAddress = locationParts.length > 2 
			? `${locationParts[0]}, ${locationParts[locationParts.length - 1]}` 
			: experience.address;

		const notificationPayload = JSON.stringify({
			title: "🔔 Broken Experience Reported Nearby",
			body: `Community member reported: "${cleanTitle}" • ${distance}km away in ${shortAddress}`,
			icon: "/favicon/web-app-manifest-192x192.png",
			badge: "/notification-icon.png", 
			image: "/images/logo.png",
			tag: `broken-experience-${experience.id}`,
			requireInteraction: false, // Don't force interaction on mobile
			silent: false,
			vibrate: [200, 100, 200], // Shorter, more pleasant pattern
			timestamp: Date.now(),
			actions: [
				{
					action: "view-details",
			...
			data: {
				...
				timestamp: Date.now(),
				category: experience.category_name,
				address: experience.address,
				title: experience.title
			},
		});
		...
		for (const subscription of subscriptions || []) {
			try {
				...
				if (error?.statusCode === 404 || error?.statusCode === 410) {
					...
					console.log(
						`🧹 Cleaned up invalid subscription for user: ${subscription.user_id}`,
					);
				}
		...
		console.log(
			`✅ Proximity notifications sent: ${notificationsSent}/${subscriptions?.length || 0}`,
		);
