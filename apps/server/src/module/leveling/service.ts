...
import { userProfile, LEVELING_CONFIG } from "@server/db/schema";
...
export class LevelingService {
	...
	static async addExperience(userId: string, experiencePoints: number) {
		...
		const currentLevelData = await tx.query.userProfile.findFirst({
			...
			if (!currentLevelData) {
				...
				const newLevel = LEVELING_CONFIG.calculateLevelFromExperience(newTotalExperience);
				const experienceToNextLevel = LEVELING_CONFIG.calculateExperienceToNextLevel(newLevel, newTotalExperience);
				...
				const newTotalExperience = currentLevelData.total_experience + experiencePoints;
				const newLevel = LEVELING_CONFIG.calculateLevelFromExperience(newTotalExperience);
				const experienceToNextLevel = LEVELING_CONFIG.calculateExperienceToNextLevel(newLevel, newTotalExperience);
	...
	static getLevelInfo(level: number) {
		const totalExperienceRequired = LEVELING_CONFIG.calculateTotalExperienceForLevel(level);
		const experienceForThisLevel = LEVELING_CONFIG.calculateExperienceForLevel(level);
		const experienceForNextLevel = LEVELING_CONFIG.calculateExperienceForLevel(level + 1);
	...
	static async getUserProgress(userId: string) {
		const userLevelData = await this.getUserLevel(userId);
		...
		const currentLevelInfo = this.getLevelInfo(userLevelData.level);
		const nextLevelInfo = this.getLevelInfo(userLevelData.level + 1);
		
		const experienceInCurrentLevel = userLevelData.total_experience - currentLevelInfo.totalExperienceRequired;
		const experienceNeededForNextLevel = nextLevelInfo.experienceForNextLevel;
		const progressPercentage = Math.floor((experienceInCurrentLevel / experienceNeededForNextLevel) * 100);
	 ...
	 */
	static async getLevelLeaderboard(limit: number = 100) {
		return await db
			.select({
