...
import { category, experience, ExperienceStatusEnum, user } from "@server/db/schema";
...
export const getUserStats = async (userId: string) => {
	const [userExperiences] = await db.select({ count: count() }).from(experience).where(eq(experience.reportedBy, userId));
	const [userResolvedExperiences] = await db.select({ count: count() }).from(experience)
		.where(and(eq(experience.reportedBy, userId), eq(experience.status, ExperienceStatusEnum.resolved)));
	const [userInProgressExperiences] = await db.select({ count: count() }).from(experience)
		.where(and(eq(experience.reportedBy, userId), eq(experience.status, ExperienceStatusEnum.in_progress)));
	...
	return {
		...
		impactScore: userExperiences.count * 10 + userResolvedExperiences.count * 25,
	};
};
