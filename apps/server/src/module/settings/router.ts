...
export const settingsRouter = new Elysia({ prefix: "/settings", tags: ["Settings"] })
	.get("/", async ({ headers }) => {
	...
	.patch("/", async ({ headers, body }) => {
		...
		const updatedSettings = await settingsService.updateUserSettings(userId, body);
	...
	.delete("/account", async ({ headers, body }) => {
		// TODO: Implement proper authentication
		const userId = "user-123";
		...
		const result = await settingsService.deleteAccount(userId, body.confirmPassword);
		return result;
	}, {
		body: t.Object({
