...
export const scoringRouter = new Elysia({ prefix: "/scoring" })
		...
		async ({ params, set }) => {
			try {
				...
				return {
					success: false,
					error:
						error instanceof Error ? error.message : "Failed to get user score",
				};
			}
		},
		{
			...
			try {
				...
				if (
					...
					(error.message.includes(
						'relation "activity_points" does not exist',
					) ||
						error.message.includes('from "activity_points"'))
				) {
					console.warn(
						"activity_points table not found. Returning empty leaderboard.",
					);
					set.status = 200;
					return {
						success: true,
						data: [],
						warning:
							"The leaderboard table is not yet set up. Please run LEADERBOARD_SQL_SETUP.sql to enable the leaderboard.",
					};
				}
		...
		async ({ params, query, set }) => {
			try {
				const formattedLeaderboard =
					await ScoringService.getCategoryLeaderboard(
						params.category,
						query.limit || 10,
						query.offset || 0,
					);
				...
				return {
					...
					error:
						error instanceof Error
							? error.message
							: "Failed to get category leaderboard",
				};
			}
		},
		{
	...
	.get("/user/:userId/rank", async ({ params, set }) => {
		try {
			...
			if (
				error instanceof Error &&
				(error.message.includes('relation "activity_points" does not exist') ||
					error.message.includes('from "activity_points"'))
			) {
				console.warn(
					"activity_points table not found. Returning default rank.",
				);
				set.status = 200;
				return {
					success: true,
			...
			return {
				success: false,
				error:
					error instanceof Error ? error.message : "Failed to get user rank",
			};
		}
	});
