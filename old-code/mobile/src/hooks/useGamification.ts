...
const LEVEL_CONFIGS: LevelConfig[] = [
  { level: 1, requiredXP: 0, title: 'New Citizen', badge: '🆕', perks: ['Can report issues'] },
  { level: 2, requiredXP: 100, title: 'Community Helper', badge: '🤝', perks: ['Can comment on issues'] },
  { level: 3, requiredXP: 300, title: 'Local Champion', badge: '🏆', perks: ['Priority issue highlighting'] },
  { level: 4, requiredXP: 600, title: 'Issue Hunter', badge: '🎯', perks: ['Advanced filtering options'] },
  { level: 5, requiredXP: 1000, title: 'Community Leader', badge: '👑', perks: ['Can moderate discussions'] },
  { level: 6, requiredXP: 1500, title: 'Change Maker', badge: '⭐', perks: ['Direct contact with authorities'] },
  { level: 7, requiredXP: 2500, title: 'Jamaica Guardian', badge: '🛡️', perks: ['Special recognition badge'] }
];
...
export const useGamification = () => {
  ...
  const getNextLevel = useCallback((currentLevel: number): LevelConfig | null => {
    return LEVEL_CONFIGS.find(config => config.level === currentLevel + 1) || null;
  }, []);

  const getProgressToNextLevel = useCallback((experience: number, currentLevel: number): number => {
    ...
    const progressRange = nextLevel.requiredXP - currentLevelConfig.requiredXP;
  ...
  const unlockAchievement = useCallback(async (achievementId: string) => {
    ...
    try {
      const achievement = await DataService.unlockAchievement(user.id, achievementId);
      if (achievement) {
        const updatedStats = {
          ...userStats,
  ...
  const updateStreak = useCallback(async () => {
    ...
    try {
      ...
      const lastActive = new Date(userStats.lastActiveDate).toISOString().split('T')[0];
  ...
  const checkAchievements = useCallback(async () => {
    ...
    const checks = [
      ...
      { id: 'first_report', condition: userStats.issuesReported >= 1, points: 50 },
      // Active reporter
      { id: 'active_reporter', condition: userStats.issuesReported >= 5, points: 100 },
      // Community engager
      { id: 'community_engager', condition: userStats.issuesUpvoted >= 10, points: 75 },
      // Consistent user
      { id: 'consistent_user', condition: userStats.streak >= 7, points: 150 },
      // Veteran user
      { id: 'veteran_user', condition: userStats.issuesReported >= 20, points: 300 }
    ];

    for (const check of checks) {
      if (check.condition && !userStats.achievements.some(a => a.id === check.id)) {
