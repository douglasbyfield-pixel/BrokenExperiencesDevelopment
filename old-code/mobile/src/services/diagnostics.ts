...
export class DiagnosticsService {
		...
		try {
			...
			if (rawIssues && rawIssues.length > 0) {
				...
				rawIssues.slice(0, 5).forEach((issue, index) => {
					console.log(`🔍 DIAGNOSTICS: Issue ${index + 1}:`, {
						...
						isValidLat:
							(issue as any).latitude !== null &&
							(issue as any).latitude !== undefined &&
							!isNaN(Number.parseFloat((issue as any).latitude)),
						isValidLng:
							(issue as any).longitude !== null &&
							(issue as any).longitude !== undefined &&
							!isNaN(Number.parseFloat((issue as any).longitude)),
					});
				});

				// Count issues with coordinates
				const withCoords = rawIssues.filter(
					(issue: any) =>
						issue.latitude !== null &&
				...
				console.log(
					"🔍 DIAGNOSTICS: Issues with coordinates:",
					withCoords.length,
					"out of",
					rawIssues.length,
				);
          ...
          `)
				...
				if (joinError) {
					...
					console.log(
						"🔍 DIAGNOSTICS: Joined issues count:",
						joinedIssues?.length || 0,
					);

					if (joinedIssues && joinedIssues.length > 0) {
						const joinedWithCoords = joinedIssues.filter(
							(issue: any) =>
								issue.latitude &&
								issue.longitude &&
								!isNaN(Number.parseFloat(issue.latitude)) &&
						...
						console.log(
							"🔍 DIAGNOSTICS: Joined issues with coordinates:",
							joinedWithCoords.length,
						);
					}
				}
	...
	static async testCoordinateValidation() {
		...
		const testIssues = [
			{ id: "1", title: "Valid coords", latitude: 18.15, longitude: -77.3 },
			{
				id: "2",
				title: "String coords",
				latitude: "18.1500",
			...
			{ id: "3", title: "Null lat", latitude: null, longitude: -77.3 },
			{
				id: "4",
				title: "NaN coords",
				latitude: Number.NaN,
		...
		testIssues.forEach((issue) => {
			const isValid =
				issue.latitude &&
				issue.longitude &&
				!isNaN(issue.latitude as any) &&
				!isNaN(issue.longitude as any);
